...
def main() -> None:
    ...
    model_A = PPO("MultiInputPolicy", env, device="cuda", verbose=0)
    ...
    end_time = time.time()
    print(f"Tempo impiegato: {int((end_time - start_time)/3600)}:{int((end_time - start_time)/60)}:{(end_time - start_time)%60:.2f}")
